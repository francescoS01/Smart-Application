<template>
  <VaSidebar v-model="writableVisible" :width="sidebarWidth" :color="color" minimized-width="0">
    <!-- Bottone per attivare/disattivare 'chatbot' e 'report' -->
    <div class="p-4">
      <VaButton @click="showSpecialRoutes = !showSpecialRoutes">
        {{ showSpecialRoutes ? 'Switch to Specialty Manufacturing Owner visualization' : 'Switch to Factory Floor Manager visualization' }}
      </VaButton>
    </div>
...
export default defineComponent({
  ...
  // Stato di visibilità per 'chatbot' e 'report'
  const showSpecialRoutes = ref(true);

  // Funzione per filtrare le route
  const filteredRoutes = computed(() =>
    navigationRoutes.routes.filter(
      (route) =>
        route.name !== "history" &&
        route.name !== "kpi" &&
        route.name !== "report" ||
        showSpecialRoutes.value
    )
  );
