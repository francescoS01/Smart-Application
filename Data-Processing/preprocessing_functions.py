...
from api_interaction import send_alert 
...
def check_invalid_data(data):
    ...
    Parameters:
    - data: DataFrame containing KPI data with columns ['time_stamp', 'machineID', 'kpi', 'aggr_type', 'value'].
    """
    for machineID in data['machineID'].unique():
        for time_stamp in data['time_stamp'].unique():

            # Check cycle consistency
            try:
                good_cycles = data[(data['time_stamp'] == time_stamp) & (data['machineID'] == machineID) & (data['kpi'] == 'good_cycles') & (data['aggr_type'] == 'sum')]['value'].values[0]
                cycles = data[(data['time_stamp'] == time_stamp) & (data['machineID'] == machineID) & (data['kpi'] == 'cycles') & (data['aggr_type'] == 'sum')]['value'].values[0]
                bad_cycles = data[(data['time_stamp'] == time_stamp) & (data['machineID'] == machineID) & (data['kpi'] == 'bad_cycles') & (data['aggr_type'] == 'sum')]['value'].values[0]
                if cycles != good_cycles + bad_cycles:
                    # fixed severity "low"
                    send_alert(machineID, "low", f'Inconsistency for {machineID} at {time_stamp}: cycle rule not respected.', 'cycles', 'sum', 'Invalid data', timestamp=None)
                    data.loc[(data['time_stamp'] == time_stamp) & (data['machineID'] == machineID) & (data['kpi'].isin(['good_cycles', 'bad_cycles', 'cycles'])) & (data['aggr_type'] == 'sum'), 'value'] = np.NaN
            except IndexError:
                print(f'Missing data for {machineID} at {time_stamp} (cycle rule)')

            # Check energy consumption consistency
            try:
                consumption_working = data[(data['time_stamp'] == time_stamp) & (data['machineID'] == machineID) & (data['kpi'] == 'consumption_working') & (data['aggr_type'] == 'sum')]['value'].values[0]
                consumption = data[(data['time_stamp'] == time_stamp) & (data['machineID'] == machineID) & (data['kpi'] == 'consumption') & (data['aggr_type'] == 'sum')]['value'].values[0]
                consumption_idle = data[(data['time_stamp'] == time_stamp) & (data['machineID'] == machineID) & (data['kpi'] == 'consumption_idle') & (data['aggr_type'] == 'sum')]['value'].values[0]
                if consumption != consumption_idle + consumption_working:
                    # fixed severity "low"
                    send_alert(machineID, "low", f'Inconsistency for {machineID} at {time_stamp}: consumption rule not respected.', 'consumption', 'sum', 'Invalid data', timestamp=None)
                    data.loc[(data['time_stamp'] == time_stamp) & (data['machineID'] == machineID) & (data['kpi'].isin(['consumption_working', 'consumption_idle', 'consumption'])) & (data['aggr_type'] == 'sum'), 'value'] = np.NaN
            except IndexError:
                print(f'Missing data for {machineID} at {time_stamp} (consumption rule)')

            # Check cost consistency
            try:
                cost_working = data[(data['time_stamp'] == time_stamp) & (data['machineID'] == machineID) & (data['kpi'] == 'cost_working') & (data['aggr_type'] == 'sum')]['value'].values[0]
                cost = data[(data['time_stamp'] == time_stamp) & (data['machineID'] == machineID) & (data['kpi'] == 'cost') & (data['aggr_type'] == 'sum')]['value'].values[0]
                cost_idle = data[(data['time_stamp'] == time_stamp) & (data['machineID'] == machineID) & (data['kpi'] == 'cost_idle') & (data['aggr_type'] == 'sum')]['value'].values[0]
                if cost != cost_idle + cost_working:
                    # fixed severity "low"
                    send_alert(machineID, "low", f'Inconsistency for {machineID} at {time_stamp}: cost rule not respected.', 'cost', 'sum', 'Invalid data', timestamp=None)
                    data.loc[(data['time_stamp'] == time_stamp) & (data['machineID'] == machineID) & (data['kpi'].isin(['cost_working', 'cost_idle', 'cost'])) & (data['aggr_type'] == 'sum'), 'value'] = np.NaN
            except IndexError:
                print(f'Missing data for {machineID} at {time_stamp} (cost rule)')
    ...
    if not np.isnan(value)[0]:  
        ...
        if len(time_series) >= card_imputation:  # Check if we have enough previous data for imputation
            ...
            if missing_ratio > 0.5:
                if send_alert_flag:
                    send_alert(streaming_data["machineID"], "high", f'Anomalous increase in missing data, more than 50% missing data for the last {card_imputation} samples', streaming_data["kpi"], streaming_data["aggr_type"], 'missing_data', timestamp=None)
            elif missing_ratio > 0.3:
                if send_alert_flag:
                    send_alert(streaming_data["machineID"], "medium", f'Anomalous increase in missing data, 30% to 50% missing data for the last {card_imputation} samples', streaming_data["kpi"], streaming_data["aggr_type"], 'missing_data', timestamp=None)
               
                # for missing data, we assume a constant severity of medium 
                imp_value = np.nan
            else:
...
def handle_missing_values_h(streaming_data, time_series, send_alert_flag = True):
    ...
    for i in range(len(time_series)):
        ...
        imputation_bool, imp_value = handle_missing_values(streaming_data, time_series.iloc[i], time_series.iloc[:i], imputation_mask[:i], send_alert_flag = send_alert_flag)
...
def detect_inconsistencies_exogenous(streaming_data, time_series, exogenous_lower_thr=None, exogenous_higher_thr=None, send_alert_flag = True):
    ...
    for i in range(len(time_series)):
        ...
        if exogenous_lower_thr is not None and time_series.iloc[i] < exogenous_lower_thr:
            ...
            if send_alert_flag:
                send_alert(streaming_data["machineID"], "low", f'Found inconsistency at index {i}: the value is lower than the min threshold', streaming_data["kpi"], streaming_data["aggr_type"], 'Exogenous_inconsistency', timestamp=None)
            ...
            inconsistencies_flag[i] = 1
            if send_alert_flag:
                send_alert(streaming_data["machineID"], "low", f'Found inconsistency at index {i}: the value is higher than the max threshold', streaming_data["kpi"], streaming_data["aggr_type"], 'Exogenous_inconsistency', timestamp=None)
            ...
            if len(time_series) > (2 + i) * selected_f:
                ...
                if p_value < 0.05:
                    ...
                    if seasonal_values.mean() < time_series[-selected_f:].mean():
                        ...
                        if send_alert_flag:
                            send_alert(streaming_data["machineID"], 'high', f'A positive trend drift was detected during the last {selected_f} samples, with respect to the previous {(i+1)*selected_f} samples', streaming_data["kpi"], streaming_data["aggr_type"], 'Positive_trend_drift', timestamp=None)
                       
                    else:
                        trend_drift = -(i + 1)
                        # Negative trend drift
                        if send_alert_flag:
                            send_alert(streaming_data["machineID"], 'high', f'A negative trend drift was detected during the last {selected_f} samples, with respect to the previous {(i+1)*selected_f} samples', streaming_data["kpi"], streaming_data["aggr_type"], 'Negative_trend_drift', timestamp=None)
...
def historical_concept_drift(streaming_data, time_series, selected_f, n = 4, send_alert_flag = True):
    ...
    for i in range(len(time_series)):
        ...
        trend_drift_h[i] = detect_trend_drift_recursive(streaming_data, h_time_series, selected_f, n, send_alert_flag = send_alert_flag)
...
def preprocess_entire_h_data(data, ex_thr_matrix, send_alert_flag=True):
    ...
    - seasonalities: Pandas DataFrame containing the seasonal frequencies 
      (`selected_f`) for each combination of 'machineID', 'kpi', and 'aggr_type'.
    """
        ...
        for kpi in kpis:
            for col in cols:
                ...
                best_decomposition, selected_f, inconsistencies_flag_vect, h_trend_drift, time_series, imputation_mask = preprocess_h_data(
                    {"machineID": name, "kpi": kpi, "aggr_type": col, "value": None},
                    time_series,
                    ex_lower_thr,
                    ex_higher_thr,
