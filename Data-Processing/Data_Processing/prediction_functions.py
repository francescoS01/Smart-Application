...
def forecast_with_ffnn(
    streaming_data, time_series, n_lags=14, hidden_units_options=[32, 64, 128], epochs=50, batch_size=16, seed=0, plot_results=False
):
    """
    ...
    if plot_results:
        ...
        # Access the values directly instead of using ['value']
        plt.plot(pd.to_datetime(time_series.index), time_series.values, label='Full Series', color='gray', linestyle='--', alpha=0.5)  
        plt.plot(pd.to_datetime(train.index), train.values, label='Train', color='blue')  
        plt.plot(pd.to_datetime(validation.index), validation.values, label='Validation', color='green')  
        plt.plot(pd.to_datetime(test.index), test.values, label='Test', color='red')  
...
def forecast_future(time_series, trained_ffnn, scaler, std_residuals, n_future=28, n_lags=14, plot_results=False):
    """
...
def forecast_from_dataframe(df, n_lags=14, n_future=28, hidden_units_options=[32, 64, 128], epochs=50, batch_size=16, seed=0, plot_results=False):
    """
    ...
    df['next_days_predictions'] = df['next_days_predictions'].astype('object')
    df['confidence_interval_lower'] = df['confidence_interval_lower'].astype('object')
    df['confidence_interval_upper'] = df['confidence_interval_upper'].astype('object')
    ...
    for (machineID, kpi, aggr_type), group in tqdm(grouped):
        ...
        if imputation_mean < 0.15:  # If the majority of values are not imputed
            ...
            # Create a combined filter based on timestamp, machineID, kpi, and aggr_type
            mask = (df['timestamp'] == last_timestamp) & \
                (df['machineID'] == machineID) & \
                (df['kpi'] == kpi) & \
                (df['aggr_type'] == aggr_type)

            # Check if at least one row matches the filter
            if mask.any():
                # Get the index of the first row that satisfies the filter
                index = df.loc[mask].index[0]
                
                # Update the specified columns with the corresponding values
                df.at[index, 'next_days_predictions'] = predictions_rescaled.tolist()
                df.at[index, 'confidence_interval_lower'] = confidence_lower.tolist()
                df.at[index, 'confidence_interval_upper'] = confidence_upper.tolist()
            else:
                # Print a warning if no rows match the filter
                print("No rows found for the specified filter!")
