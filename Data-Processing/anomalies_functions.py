...
def detect_anomalies_from_decomposition(streaming_data, residuals, contamination="auto", plot_flag=False, send_alert_flag = True):
    ...
    for idx in anomaly_indices:
        if send_alert_flag:
            ...
            send_alert(streaming_data["machineID"], "medium", "anomaly detected in historical data", streaming_data["kpi"], streaming_data["aggr_type"], 'Anomaly_detected', timestamp=None)
        # we should use different timestamp!!!! E.g. timestamp = pd.to_datetime(residuals.index[idx])
        #############################################
...
def detect_anomalies_from_dataframe(df, contamination="auto", plot_flag=False, send_alert_flag=True):
    ...
    df['anomaly'] = False  # Default value: no anomalies
    ...
    for (machineID, kpi, aggr_type), group in grouped:
        ...
        if imputation_mean < 0.15:  # If the majority of values are not imputated
            ...
            anomaly_mask, iso_forest = detect_anomalies_from_decomposition(
                streaming_data,
                residuals,
